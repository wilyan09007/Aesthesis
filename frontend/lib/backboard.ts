// Shared Backboard client + Aesthesis-specific assistant config. Both
// /api/agent (chat) and /api/agent/insight-fix (single-insight suggestions)
// use these helpers so we don't duplicate the assistant/threading logic.

import { prisma } from "@/lib/prisma"
import { listRuns, compareRuns } from "@/lib/db/runs"
...
export async function bb(path: string, body?: unknown): Promise<Response> {
...
export async function getAssistantId(): Promise<string> {
  ...
  console.log("[backboard] create assistant:", JSON.stringify(data).slice(0, 200))
...
export async function createEphemeralThread(assistantId: string): Promise<string> {
  const res = await bb(`/assistants/${assistantId}/threads`, {})
  const data = await res.json()
  return data.thread_id as string
}

// One Backboard thread per saved run — all chat sessions about the same run
// share context across server restarts.
export async function getOrCreateThreadForRun(assistantId: string, runId: string): Promise<string> {
  ...
  const threadId = await createEphemeralThread(assistantId)
...
export async function runLoop(
  ...
  console.log("[backboard] message response status:", res.status)
  ...
  while (res.status === "REQUIRES_ACTION" && iterations < 8) {
    ...
    console.log(`[backboard] iter ${iterations} new calls:`, newCalls.map((c) => c.function.name))
    ...
    const outputs = await Promise.all(
      newCalls.map(async (call) => {
        ...
        console.log(`[backboard] tool ${call.function.name}:`, JSON.stringify(result).slice(0, 150))
    ...
    console.log("[backboard] submit response status:", res.status)
  }
  console.log("[backboard] final:", res.status, "reply length:", res.content?.length ?? 0)
...
// Builds the executeTool callback for runLoop — scoped to a specific user
// (for past-run lookups) and optional currentRunId (for comparisons).
export function buildExecuteTool(opts: {
  user: { id: string }
  currentRunId?: string | null
}) {
  const { user, currentRunId } = opts
