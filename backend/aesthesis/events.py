...
We cap the number of events at ``EVENT_CAP`` (default 15 — the upper end of
the §4.5 "8-15 events per 60s" target). When we exceed the cap, we keep
the highest-magnitude events and the head of each event class to preserve
diversity.

Pre-pivot this took a ``version: VersionTag`` arg and tagged every event
with "A" or "B". The pivot dropped the version dimension (DESIGN.md §17).
"""
...
def _trough_event(frame: dict) -> Event | None:
...
def _spike_events(frame: dict) -> Iterable[Event]:
...
def _dominant_shift_event(frame: dict, prev_dominant: str | None) -> Event | None:
...
def _sustained_events(frames: list[dict]) -> list[Event]:
...
def _co_movement_events(frames: list[dict]) -> list[Event]:
...
def _window_events(windows: list[dict]) -> list[Event]:
...
def extract_events(timeline: dict) -> list[Event]:
    ...
    Caps the output at EVENT_CAP, preserving:
        ...
        3. the highest-magnitude ``spike`` / ``co_movement`` /
           ``dominant_shift`` / ``sustained`` events to fill the rest.
    """
    frames: list[dict] = timeline.get("frames", [])
    windows: list[dict] = timeline.get("windows", [])
    log.debug(
        "extract_events begin",
        extra={"step": "events", "n_frames": len(frames), "n_windows": len(windows)},
    )
    ...
    for frame in frames:
        raw.extend(_spike_events(frame))
        ds = _dominant_shift_event(frame, prev_dominant)
        if ds is not None:
            raw.append(ds)
        prev_dominant = frame.get("dominant")
        t = _trough_event(frame)
        if t is not None:
            raw.append(t)

    raw.extend(_sustained_events(frames))
    raw.extend(_co_movement_events(frames))
    raw.extend(_window_events(windows))
    ...
    if len(raw) <= EVENT_CAP:
        log.info(
            "events extracted",
            extra={"step": "events", "n_events": len(raw)},
        )
        return raw
    ...
    log.info(
        ...
        extra={"step": "events", "n_events": len(keep), "n_raw": len(raw)},
    )
    return keep
